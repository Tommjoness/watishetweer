...
(function(){
  ...
  window.__wiwPlan={mode:'success',delay:0,perLat:null};window.__wiwOffline=false;
  const echteSetTimeout=window.setTimeout.bind(window);window.__wiwEchteSetTimeout=echteSetTimeout;
  window.setTimeout=function(fn,ms){const args=[].slice.call(arguments,2);const versneld=ms===10000?45:ms===7000?35:ms;return echteSetTimeout(()=>fn.apply(null,args),versneld);};
...
(async()=>{
 ...
 const reset=()=>{try{clearNuTimer();clearKlokTimer();}catch(e){};S.lat=null;S.lon=null;S.label='';S.land=null;S.d=null;S.air=null;S.op=0;S.luchtOp=0;S.dag=null;S.verversMislukt=false;S.actieveWaarschuwingen=[];app.style.display='none';st.style.display='none';q.value='';};
 ...
 try{
   ...
   zet('provider',goed('Amsterdam',52.3676,4.9041)&&/Kansas City/.test(st.textContent)&&!!st.querySelector('.wiw-location-retry')?'ok':'fout');

   reset();ls.set(KEY_D,null);await laadGoed('Amsterdam',52.3676,4.9041,0);q.value='Kathmandu';window.__wiwOffline=true;window.__wiwPlan={mode:'provider-error',delay:0,perLat:null};await load(27.7172,85.3240,'Kathmandu',false,true,'NP');await slaap(15);
   zet('offline',goed('Amsterdam',52.3676,4.9041)&&/Geen internetverbinding/.test(st.textContent)&&/Kathmandu/.test(st.textContent)?'ok':'fout');window.__wiwOffline=false;
   ...
   zet('same-cache',goed('Kansas City',39.0997,-94.5786)&&S.verversMislukt&&app.style.display!=='none'&&/laatst opgehaalde gegevens voor Kansas City/.test(st.textContent)?'ok':'fout');
   ...
   zet('wrong-cache',S.d===null&&S.label==='Kansas City'&&q.value==='Kansas City'&&app.style.display==='none'&&/geen weergegevens van een andere locatie/.test(st.textContent)?'ok':'fout');

   reset();ls.set(KEY_D,null);window.__wiwPlan={mode:'success',delay:0,perLat:{'52.3676':{mode:'success',delay:110},'39.0997':{mode:'success',delay:70},'27.7172':{mode:'success',delay:15}}};
   q.value='Amsterdam';const p1=load(52.3676,4.9041,'Amsterdam',false,true,'NL');await slaap(5);q.value='Kansas City';const p2=load(39.0997,-94.5786,'Kansas City',false,true,'US');await slaap(5);q.value='Kathmandu';const p3=load(27.7172,85.3240,'Kathmandu',false,true,'NP');await Promise.allSettled([p1,p2,p3]);await slaap(130);
   ...
   zet('direct-wrong-cache',S.d===null&&S.label==='Kansas City'&&q.value==='Kansas City'&&app.style.display==='none'&&document.title.startsWith('Kansas City · ')?'ok':'fout');
