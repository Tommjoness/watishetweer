...
}
{
 ...
 api.briefing();const t=tekst(bak.brief);ok(/Het verwachte maximum ligt vandaag rond \d\d:\d\d op 31 graden/.test(t),"na lokale middernacht heet de nieuwe dag vandaag",t);ok(!/Het verwachte maximum (?:ligt morgen|voor morgen is)/.test(t),"stale current.time houdt morgen niet vast",t);
}
{
 ...
 ok(/Het verwachte maximum ligt vandaag rond \d\d:\d\d op 35 graden/.test(t),"om 14:02 blijft de briefing bij de resterende huidige dag",t);
 ok(!/Het verwachte maximum (?:ligt morgen|voor morgen is)/.test(t),"om 14:02 loopt de briefing niet vooruit op morgen",t);
}
{
...
}
{
 ...
 api.briefing();const t=tekst(bak.brief);
 ok(/Het verwachte maximum ligt morgen rond \d\d:\d\d op 38 graden/.test(t),"in de avond gebruikt morgen het volledige kalenderdagmaximum",t);
