...
function zetBasis(api,d,extra){
  ...
  Object.assign(api.S,{d,i0:i,op:Date.now(),lat:52.35,lon:5.26,label:"Productietest",dag:null,bereik:24,
    klokOverride:new Date("2026-07-22T12:00:00Z"),klokInstantOverride:null},extra||{});
  return i;
}
...

{
  ...
  Object.assign(api.S,{d,i0:i,op:Date.now(),lat:52.35,lon:5.26,label:"Productietest",dag:null,bereik:24,
    klokOverride:new Date("2026-07-22T22:05:00Z"),klokInstantOverride:null});
...

{
  ...
  const i=d.hourly.time.findIndex(t=>t==="2026-07-22T23:00");
  Object.assign(api.S,{d,i0:i,op:Date.now(),lat:52.35,lon:5.26,label:"Productietest",dag:null,bereik:24,
    klokOverride:new Date("2026-07-22T21:55:00Z"),klokInstantOverride:null});
...
/* DST gebruikt een absoluut UTC-moment; de gewone klokOverride blijft gereserveerd
   voor de historische lokale-klokfixtures van de bestaande wereldmatrix. */
{
  const {api}=laadKern(390); const d=bouw({}); d.timezone="Europe/Amsterdam"; d.utc_offset_seconds=7200; zetBasis(api,d);
  api.S.klokOverride=null; api.S.klokInstantOverride=new Date("2026-10-25T02:30:00Z");
