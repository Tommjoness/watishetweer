...
async function check(type,naam,breedte){
  ...
  try{
    ...
    const init=await page.evaluate(()=>{const k=document.getElementById("thema"),m=document.getElementById("themamenu"),f=document.querySelector('link[rel="icon"]'),s=getComputedStyle(k),t=getComputedStyle(document.querySelector(".tools"));return {tekst:k.textContent.trim(),hidden:m.hidden,keuzes:[...m.querySelectorAll("[data-thema-keuze]")].map(x=>x.dataset.themaKeuze),fav:f&&f.getAttribute("href"),past:k.scrollWidth<=k.clientWidth+1,clientWidth:k.clientWidth,scrollWidth:k.scrollWidth,fontSize:s.fontSize,letterSpacing:s.letterSpacing,paddingLeft:s.paddingLeft,paddingRight:s.paddingRight,toolsDisplay:t.display,toolsWidth:document.querySelector(".tools").clientWidth};});
    assert.equal(init.tekst,"Weergave",naam+": compacte duidelijke knop");assert.equal(init.hidden,true,naam+": menu gesloten");assert.deepEqual(init.keuzes,["auto","licht","donker"],naam+": alleen nuttige keuzes");assert(init.fav&&init.fav.startsWith("data:image/svg+xml,"),naam+": zon-favicon");assert(init.past,naam+": knop past op "+breedte+"px; layout="+JSON.stringify({clientWidth:init.clientWidth,scrollWidth:init.scrollWidth,fontSize:init.fontSize,letterSpacing:init.letterSpacing,paddingLeft:init.paddingLeft,paddingRight:init.paddingRight,toolsDisplay:init.toolsDisplay,toolsWidth:init.toolsWidth}));
