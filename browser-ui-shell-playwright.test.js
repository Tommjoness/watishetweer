...
async function check(type,naam,breedte){
  ...
  try{
    ...
    assert.equal(init.tekst,"Weergave",naam+": compacte duidelijke knop");assert.equal(init.hidden,true,naam+": menu gesloten");assert.deepEqual(init.keuzes,["auto","licht","donker"],naam+": alleen nuttige keuzes");assert(init.fav&&init.fav.startsWith("data:image/svg+xml,"),naam+": zon-favicon");assert(init.past,naam+": knop past op "+breedte+"px");
