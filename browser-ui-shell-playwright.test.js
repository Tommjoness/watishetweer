...
async function check(type,naam,breedte){
  ...
  try{
    ...
    assert.equal(init.tekst,"Weergave · auto",naam+": duidelijke knop");assert.equal(init.hidden,true,naam+": menu gesloten");assert.deepEqual(init.keuzes,["auto","licht","donker","rood"],naam+": unieke keuzes");assert(init.fav&&init.fav.startsWith("data:image/svg+xml,"),naam+": zon-favicon");assert(init.past,naam+": knop past op "+breedte+"px");
    await page.click("#thema");assert.equal(await page.locator("#themamenu").evaluate(x=>x.hidden),false,naam+": menu opent");
    await page.click('[data-thema-keuze="donker"]');assert.equal(await page.evaluate(()=>document.documentElement.dataset.thema),"donker",naam+": donker selecteert");assert.equal(await page.textContent("#thema"),"Weergave · donker");
    ...
    assert.deepEqual(nacht,{actief:"donker",knop:"Weergave · auto",auto:"true",donker:"false"},naam+": auto-nacht is geen tweede donkerkeuze");assert.deepEqual(errors,[],naam+": geen page errors");
