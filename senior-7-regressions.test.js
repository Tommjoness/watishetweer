...
const index=lees("index.html"),engine=lees("interpretatie-engine.js"),build=lees("build-weather.js"),waars=lees("lib/waarschuwingen.cjs"),config=lees("product-config.js"),correct=lees("senior-correctness-v2.js");
ok(build.includes('require("./product-config.js")')&&config.includes("EERSTE_BEZOEK_PRODUCTIE")&&config.includes('naam:"Amsterdam"'),"bewuste productsemantiek staat expliciet in één productconfiguratie");
ok(build.includes("SENIOR CORRECTHEIDSLAAG")&&correct.includes("WeatherNowCorrectnessV2"),"senior-correctheidslaag wordt expliciet in productie geassembleerd");
ok(!fs.existsSync(path.join(R,"productie-hardening-v2.js")),"oude ondoorzichtige semantische hardeninglaag is verwijderd");
...
ok(index.includes("Komend uur")&&engine.includes("de komende circa 60 minuten")&&engine.includes("overlappende uurvakken"),"uurkans benoemt de modelresolutie zonder schijnprecisie");
...
ok(engine.includes('w.niveauIsOfficieel===false')&&engine.includes('Officiële weerwaarschuwing"+(w.niveau'),"waarschuwingskop is grammaticaal correct en onderscheidt bronsemantiek");
(async()=>{
