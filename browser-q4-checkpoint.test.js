...
async function apiStress(browser,naam){
  ...
  try{
    ...
    await page.waitForFunction(()=>/konden niet worden gecontroleerd/i.test(document.getElementById('waarschuwingen').textContent));
    ...
    assert(/konden niet worden gecontroleerd/i.test(degradatie.warn),naam+": waarschuwingfout wordt eerlijk gemeld");
