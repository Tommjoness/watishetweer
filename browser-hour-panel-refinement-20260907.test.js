...
document.addEventListener('DOMContentLoaded',async()=>{const zet=(k,v)=>document.body.setAttribute('data-hour-refine-'+k,String(v));try{
 ...
 const placeStyle=place?getComputedStyle(place):null;
...
try{
  ...
  // #place krijgt de binnenruimte zelf. Range-/tekstrects zijn hier ongeschikt:
  // een block heading rapporteert zijn contentbox los van de toegepaste padding.
  // De berekende browserstijl bewijst rechtstreeks dat beide kanten de finale
  // desktop-inset hebben en voorkomt een test die de juiste CSS als 0 px meet.
