...
try{
  ...
  for(const prefix of ['base','grown']){
    ...
    if(Number(v(prefix+'-graph-count'))!==Number(v(prefix+'-rows'))||v(prefix+'-graph-first')!=='2026-09-02T14:00')throw new Error(`${prefix}: grafiek en tabel delen niet exact dezelfde zichtbare desktoprange`);
  ...
  console.log(`Desktoprefinement groen op 1600×900: natuurlijke grafiek links en rijke Komende-uren-tabel rechts met ${basis} volledige regels (${groter} bij +80px), exact dezelfde grafiek-/tabelrange, echte weer/gevoel/neerslag/winddata en compacte Nachtzicht-groepering.`);
