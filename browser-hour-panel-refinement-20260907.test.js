...
document.addEventListener('DOMContentLoaded',async()=>{const zet=(k,v)=>document.body.setAttribute('data-hour-refine-'+k,String(v));try{
 ...
 for(const [prefix,m] of [['base',basis],['grown',groter]]){zet(prefix+'-rows',m.count);zet(prefix+'-first',m.first);zet(prefix+'-last',m.last);zet(prefix+'-panel-height',m.panelHeight.toFixed(1));zet(prefix+'-main-height',m.mainHeight.toFixed(1));zet(prefix+'-fits',m.lastBottom<=m.panelBottom+1?'ok':'fout');zet(prefix+'-table-tight',Math.abs(m.tableBottom-m.lastBottom)<=2?'ok':'fout');zet(prefix+'-panel-fill',Math.abs(m.panelBottom-m.tableBottom)<=2?'ok':'fout');zet(prefix+'-row-min',m.rowHeights.length?Math.min(...m.rowHeights).toFixed(1):'0');zet(prefix+'-row-max',m.rowHeights.length?Math.max(...m.rowHeights).toFixed(1):'0');zet(prefix+'-overflow',m.overflow);zet(prefix+'-title',m.title);}
...
try{
  ...
  for(const prefix of ['base','grown']){
    ...
    if(Number(v(prefix+'-row-min'))<18||Number(v(prefix+'-row-max'))>24)throw new Error(`${prefix}: uurregels zijn te smal of te ruim (${v(prefix+'-row-min')}–${v(prefix+'-row-max')}px)`);
  ...
  console.log(`Desktoprefinement groen op 1600×900: ${basis} volledige uurregels op natuurlijke hoogte en ${groter} na +80px; rijcount is hoogtegestuurd, resterende ruimte wordt gelijkmatig over leesbare rijen verdeeld, paneelhoogtes sluiten aan en er is geen interne scrollbar. Exacte grafiek/tabel-bronuren worden door de CWV-/klokgates met volledige grafiekstate bewaakt.`);
