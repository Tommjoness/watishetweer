...
document.addEventListener('DOMContentLoaded',async()=>{const zet=(k,v)=>document.body.setAttribute('data-hour-refine-'+k,String(v));try{
 ...
 for(const [prefix,m] of [['base',basis],['grown',groter]]){zet(prefix+'-rows',m.count);zet(prefix+'-first',m.first);zet(prefix+'-last',m.last);zet(prefix+'-panel-height',m.panelHeight.toFixed(1));zet(prefix+'-main-height',m.mainHeight.toFixed(1));zet(prefix+'-layout-width',m.layoutWidth.toFixed(1));zet(prefix+'-main-width',m.mainWidth.toFixed(1));zet(prefix+'-fits',m.lastBottom<=m.panelBottom+1?'ok':'fout');zet(prefix+'-table-tight',Math.abs(m.tableBottom-m.lastBottom)<=2?'ok':'fout');zet(prefix+'-panel-fill',Math.abs(m.panelBottom-m.tableBottom)<=2?'ok':'fout');zet(prefix+'-row-min',m.rowHeights.length?Math.min(...m.rowHeights).toFixed(1):'0');zet(prefix+'-row-max',m.rowHeights.length?Math.max(...m.rowHeights).toFixed(1):'0');zet(prefix+'-overflow',m.overflow);zet(prefix+'-panel-title',m.panelTitle);zet(prefix+'-panel-visibility',m.panelVisibility);zet(prefix+'-layout-columns',m.layoutColumns);zet(prefix+'-chart-title',m.chartTitle);}
...
try{
  ...
  if(!(basis>=10&&basis<=kandidaten))throw new Error(`natuurlijke volle-breedtegrafiek levert onredelijke uurset: rows=${basis} candidates=${kandidaten}`);
  ...
  for(const prefix of ['base','grown']){
    ...
    if(v(prefix+'-fits')!=='ok'||v(prefix+'-table-tight')!=='ok'||v(prefix+'-panel-fill')!=='ok')throw new Error(`${prefix}: technische uurbron bevat halve/lege onderste rij of overflow`);
    if(Math.abs(Number(v(prefix+'-panel-height'))-Number(v(prefix+'-main-height')))>1)throw new Error(`${prefix}: technische uurbron en grafiekhoogte sluiten niet aan (${v(prefix+'-panel-height')} vs ${v(prefix+'-main-height')})`);
    if(Math.abs(Number(v(prefix+'-layout-width'))-Number(v(prefix+'-main-width')))>2)throw new Error(`${prefix}: grafiek benut niet de volledige layoutbreedte (${v(prefix+'-main-width')} van ${v(prefix+'-layout-width')})`);
    if(Number(v(prefix+'-row-min'))<18||Number(v(prefix+'-row-max'))>24)throw new Error(`${prefix}: technische uurregels zijn te smal of te ruim (${v(prefix+'-row-min')}–${v(prefix+'-row-max')}px)`);
    if(v(prefix+'-overflow')!=='visible')throw new Error(`${prefix}: interne desktopscrollbar is niet uitgeschakeld (${v(prefix+'-overflow')})`);
    if(v(prefix+'-panel-title')!=='Temperatuur en neerslag per uur')throw new Error(`${prefix}: technische uurtitel gewijzigd`);
    if(v(prefix+'-panel-visibility')!=='hidden')throw new Error(`${prefix}: oude zichtbare uurkolom is nog in beeld (${v(prefix+'-panel-visibility')})`);
  ...
  console.log(`Desktoprefinement groen op 1600×900: Komende uren gebruikt de volle grafiekbreedte; de oude uurkolom is visueel verborgen maar houdt ${basis} exact gekoppelde volledige bronuren (${groter} bij +80px), zonder interne scrollbar.`);
