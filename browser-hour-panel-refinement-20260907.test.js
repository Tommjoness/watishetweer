...
document.addEventListener('DOMContentLoaded',async()=>{const zet=(k,v)=>document.body.setAttribute('data-hour-refine-'+k,String(v));try{
 ...
 /* Deze fixture is bewust alleen eigenaar van uurpaneelgeometrie. Laat S.geo.x
    weg zodat de echte etmaalrenderer niet met een kunstmatig onvolledige
    grafiekstate wordt aangeroepen. Exacte grafiek/tabel-bronuren worden in de
    production-style CWV- en klokgates met volledige S.geo-state geverifieerd. */
 ...
 const meet=()=>{const rows=[...document.querySelectorAll('#wiw-hour-table tbody tr')],panel=document.getElementById('wiw-hour-panel'),main=document.querySelector('.wiw-chart-main'),dagmod=main?.querySelector('.dagmod'),scroll=document.getElementById('wiw-hour-scroll'),table=document.getElementById('wiw-hour-table'),last=rows.at(-1),pr=panel?.getBoundingClientRect(),mr=main?.getBoundingClientRect(),dr=dagmod?.getBoundingClientRect(),tr=table?.getBoundingClientRect(),lr=last?.getBoundingClientRect();return {count:rows.length,first:rows[0]?.querySelector('time')?.textContent.trim()||'',last:last?.querySelector('time')?.textContent.trim()||'',mm:rows.map(r=>r.children[3]?.textContent.trim()||''),panelHeight:pr?.height||0,mainHeight:mr?.height||0,mainInnerGap:mr&&dr?mr.bottom-dr.bottom:999,lastBottom:lr?.bottom||0,panelBottom:pr?.bottom||0,tableBottom:tr?.bottom||0,rowHeights:rows.map(r=>r.getBoundingClientRect().height),overflow:scroll?getComputedStyle(scroll).overflowY:'',title:document.getElementById('wiw-hour-title')?.textContent||''};};
 ...
 const basis=meet();const main=document.querySelector('.wiw-chart-main'),panel=document.getElementById('wiw-hour-panel'),natuurlijk=main?.getBoundingClientRect().height||0;
 /* Reproduceer de productieval expliciet: een oude te grote paneelhoogte mag
    bij de volgende render niet als nieuwe grafiekhoogte terugkeren. */
 if(panel)panel.style.height=(natuurlijk+180)+'px';WeatherNowFinalDesktopUI20260902.render();await wacht();const naOudePaneelhoogte=meet();
 if(main)main.style.height=(natuurlijk+80)+'px';WeatherNowFinalDesktopUI20260902.render();await wacht();const groter=meet();
 if(main)main.style.height='';WeatherNowFinalDesktopUI20260902.render();await wacht();const hersteld=meet();
 ...
 for(const [prefix,m] of [['base',basis],['stale',naOudePaneelhoogte],['grown',groter],['restored',hersteld]]){zet(prefix+'-rows',m.count);zet(prefix+'-first',m.first);zet(prefix+'-last',m.last);zet(prefix+'-panel-height',m.panelHeight.toFixed(1));zet(prefix+'-main-height',m.mainHeight.toFixed(1));zet(prefix+'-main-inner-gap',m.mainInnerGap.toFixed(1));zet(prefix+'-fits',m.lastBottom<=m.panelBottom+1?'ok':'fout');zet(prefix+'-table-tight',Math.abs(m.tableBottom-m.lastBottom)<=2?'ok':'fout');zet(prefix+'-panel-fill',Math.abs(m.panelBottom-m.tableBottom)<=2?'ok':'fout');zet(prefix+'-row-min',m.rowHeights.length?Math.min(...m.rowHeights).toFixed(1):'0');zet(prefix+'-row-max',m.rowHeights.length?Math.max(...m.rowHeights).toFixed(1):'0');zet(prefix+'-overflow',m.overflow);zet(prefix+'-title',m.title);}
...
try{
  ...
  const r=spawnSync(browser,["--headless=new","--no-sandbox","--disable-gpu","--disable-dev-shm-usage","--allow-file-access-from-files","--window-size=1600,900","--virtual-time-budget=4500","--dump-dom","file://"+pad],{encoding:"utf8",maxBuffer:36*1024*1024});
  ...
  const basis=Number(v('base-rows')),stale=Number(v('stale-rows')),groter=Number(v('grown-rows')),hersteld=Number(v('restored-rows'));
  ...
  if(stale!==basis)throw new Error(`oude paneelhoogte beïnvloedt nog de uurselectie: basis=${basis} stale=${stale}`);
  if(!(groter>basis&&groter<=Number(v('candidates'))))throw new Error(`zichtbare uren reageren niet op extra beschikbare grafiekhoogte: basis=${basis} groter=${groter}`);
  if(hersteld!==basis)throw new Error(`uurselectie herstelt niet naar natuurlijke grafiekhoogte: basis=${basis} restored=${hersteld}`);
  for(const prefix of ['base','stale','grown','restored']){
    if(v(prefix+'-first')!=='14:00')throw new Error(`${prefix}: eerste uur verschoof onverwacht naar ${v(prefix+'-first')}`);
    if(v(prefix+'-fits')!=='ok'||v(prefix+'-table-tight')!=='ok'||v(prefix+'-panel-fill')!=='ok')throw new Error(`${prefix}: halve/lege onderste rij, loos ondervlak of overflow`);
    ...
    if(Number(v(prefix+'-row-min'))<24||Number(v(prefix+'-row-max'))>30)throw new Error(`${prefix}: uurregels zijn gepropt of te ruim (${v(prefix+'-row-min')}–${v(prefix+'-row-max')}px)`);
  ...
  for(const prefix of ['base','stale','restored'])if(Math.abs(Number(v(prefix+'-main-inner-gap')))>2)throw new Error(`${prefix}: grafiekcontainer bevat opnieuw kunstmatige verticale rek (${v(prefix+'-main-inner-gap')}px)`);
  ...
  console.log(`Desktoprefinement groen op 1600×900: ${basis} comfortabele volledige uurregels op natuurlijke hoogte, oude paneelhoogte verandert de selectie niet, ${groter} regels na +80px en ${hersteld} na herstel; rijcount is werkelijk grafiekhoogtegestuurd, paneel en grafiek sluiten aan en er is geen interne scrollbar.`);
