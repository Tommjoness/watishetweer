...
async function haalKnmi({ lat, lon, fetchImpl = fetch, nuMs = Date.now() }) {
  ...
  const nowcastFout = foutreden(nowcastResultaat);
  if (!actueel && !nowcast) {
    const redenen = [actueelFout, nowcastFout].filter(Boolean);
    return {
      beschikbaar: false,
      provider: "knmi",
  ...
  const degradaties = {};
  if (actueelFout) degradaties.actueel = actueelFout;
  if (nowcastFout) degradaties.nowcast = nowcastFout;
  ...
  return {
    ...
    actueel,
    nowcast,
    ...(Object.keys(degradaties).length ? { degradaties } : {}),
