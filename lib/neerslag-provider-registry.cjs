...
async function haalNeerslagVoorLocatie({ lat, lon, land, fetchImpl = fetch, nuMs = Date.now() }) {
  ...
  /* Bestaande productieclients sturen nog geen landcode mee en vragen deze route
     uitsluitend voor Nederlandse locaties op. Daarom blijft selectie op KNMI-
     dekking tijdelijk backwards-compatible. Nieuwe providers moeten wel via hun
     landcode worden geselecteerd voordat de client voor die landen wordt geopend. */
