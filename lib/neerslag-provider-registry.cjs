...
const PROVIDERS = Object.freeze([
  Object.freeze({
    ...
    ondersteunt({ lat, lon, land }) {
      return land === "NL" && knmi.binnenKnmiDekking(lat, lon);
    },
    haal: haalKnmi
  })
]);
...
function kiesProvider({ lat, lon, land }) {
  ...
  if (!Number.isFinite(y) || !Number.isFinite(x) || !cc) return null;
...
async function haalNeerslagVoorLocatie({ lat, lon, land, fetchImpl = fetch, nuMs = Date.now() }) {
  ...
  if (!cc) {
    return { beschikbaar: false, provider: null, reden: "landcode ontbreekt" };
  }
