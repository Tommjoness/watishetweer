...
async function viaMeteoAlarm(slug, lat, lon) {
  ...
  if (gecachetCompat) {
    try {
      return { bron: compat, lijst: uitCap(JSON.parse(gecachetCompat), lat, lon), plaatsSpecifiek: true };
    } catch (e) {}
  }
  ...
  try {
    ...
    if (kop === "{" || kop === "[") {
      const lijst = uitCap(JSON.parse(tekst), lat, lon);
...
module.exports._intern = { NWS_GEBIED, NWS_LANDCODES, inLengtegraadBereik, inNWS, isNWSLandCode, METEO_FEED_CACHE_TTL_S };