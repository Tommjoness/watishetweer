...
export function geldigeGenormaliseerdeForecast(data) {
  ...
  if (!data.timezone || !Number.isFinite(Number(data.utc_offset_seconds))) return false;
  if (!data.current.time || !Number.isFinite(Number(data.current.temperature_2m)) || !Number.isFinite(Number(data.current.weather_code))) return false;
