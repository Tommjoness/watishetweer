...
/* Servergrens voor officiële waarschuwingen.
   ...
   Eén uitzondering is semantisch wél bewijsbaar: als een officieel opgehaalde
   landfeed succesvol is maar nul actieve waarschuwingen bevat, zijn er ook voor
   de gekozen plaats geen actieve waarschuwingen. Scope-onzekerheid speelt pas
   wanneer er daadwerkelijk waarschuwingen aan een gebied gekoppeld moeten worden. */
function alleenPlaatsgebonden(data){
  ...
  if(bronIsNietPlaatsSpecifiek&&lijst.length===0){
    return Object.assign({},data,{dekking:true,lijst:[],plaatsSpecifiek:false});
  }
