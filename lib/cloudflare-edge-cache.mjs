...
/* Beide server-fallbacks van /api/forecast leveren hetzelfde genormaliseerde
   contract. Visual Crossing is de eerste keuze; zonder deze provider in de set
   ging iedere Visual Crossing-response als BYPASS opnieuw naar de betaalde API. */
const FORECAST_PROVIDERS = new Set(["visualcrossing", "weatherapi"]);
...
const ROUTES = Object.freeze({
  ...
  forecast: Object.freeze({
    ...
    cachebaar(body) {
      return Boolean(
        body
        && FORECAST_PROVIDERS.has(body.provider)
        && !body.reden
        && body.current
        && body.hourly
        && Array.isArray(body.hourly.time)
        && body.hourly.time.length >= 7 * 23
        && body.daily
        && Array.isArray(body.daily.time)
        && body.daily.time.length === 7
        && volledigeForecastDekking(body.hourly.time, body.daily.time)
      );
    }
  }),
  ...
  luchtkwaliteit: Object.freeze({
    /* Zelfde ~110 m-raster als de bestaande geheugencache in luchtmeetnet-lki.cjs.
       Alleen een verse LKI of de vaste "niet in Nederland"-uitkomst is cachebaar;
       een Luchtmeetnet-timeout of -storing gaat nooit de edge-cache in. */
    coordDecimals: 3,
    gebruiktLand: true,
    cachebaar(body) {
      if (!body) return false;
      if (body.beschikbaar === true) {
        return body.provider === "luchtmeetnet" && Number.isFinite(Number(body.lki)) && !body.reden;
      }
      return body.beschikbaar === false && body.provider == null
        && body.reden === "geen Nederlandse LKI voor deze locatie";
    }
  }),
...
function volledigeForecastDekking(uren, dagen) {
...
export function canoniekeCacheUrl(request, routeNaam) {
  ...
  if (latRuw === null || lonRuw === null || latRuw.trim() === "" || lonRuw.trim() === "") return null;
  const lat = Number(latRuw);
  const lon = Number(lonRuw);
...
function logCacheFout(routeNaam, operation, error) {
  try {
    console.warn({
      ...
      route: String(routeNaam || "onbekend"),
      operation: String(operation || "onbekend"),
