...
const ROUTES = Object.freeze({
  plaatsnaam: Object.freeze({
    ...
    gebruiktLand: false,
    cachebaar(body) {
      return Boolean(body && typeof body.naam === "string" && body.naam.trim() && !body.reden);
    }
  }),
...
export async function metEdgeCache(context, routeNaam, maakResponse) {
  ...
  const cachebaar = matchGewerkt && ttl > 0 && await responseCachebaar(routeNaam, response);
  if (!cachebaar) return metCacheStatus(response, "BYPASS");
  ...
  const schrijf = Promise.resolve(cache.put(key, kopie)).catch(error => {
    console.warn(`[edge-cache:${routeNaam}] cache.put mislukt`, error);
  });
  if (context && typeof context.waitUntil === "function") context.waitUntil(schrijf);
  else await schrijf;
