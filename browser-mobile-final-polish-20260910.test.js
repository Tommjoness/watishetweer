...
document.addEventListener('DOMContentLoaded',()=>{const zet=(k,v)=>document.body.setAttribute('data-mobile-final-'+k,String(v));try{
  ...
  const over=document.querySelector('footer a[href="/over/"]');
  zet('over-nowrap',over?getComputedStyle(over).whiteSpace:'');zet('over-text',over?.textContent.trim()||'');
...
try{
  ...
  for(const [w,h] of [[320,900],[390,900],[430,932]]){
    ...
    if(v('over-nowrap')!=='nowrap')throw new Error(`${w}px: compacte Over-link kan nog afbreken (${v('over-nowrap')})`);
    if(v('over-text')!=='Over deze site')throw new Error(`${w}px: footerlink heeft niet de gewenste compacte copy (${v('over-text')})`);
    ...
    console.log(`${w}px mobiele final-polish groen: eerste komende uur 14:00, 23 rijen, gekozen dag 24 rijen, titel ${v('title-size')}px, headerfade ${v('fade-height')}px, compacte Over-link aanwezig.`);
  }
  console.log("Mobiele final-polish browsertest geslaagd op 320/390/430 px, inclusief compacte Over-link zonder overflow.");
