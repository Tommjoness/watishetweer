...
window.fetch=function(url){
  ...
  if(u.includes('geocoding-api.open-meteo.com')){
    return new Promise(resolve=>setTimeout(()=>resolve(${antwoord({results:[{name:"Amsterdam",latitude:52.3676,longitude:4.9041,country_code:"NL",admin1:"Noord-Holland"}]})}),700));
  }
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  setTimeout(()=>{
    try{
      const ok=melding&&melding.classList.contains('on')&&melding.textContent.trim()==='Plaatsen zoeken…'
        &&res&&!res.classList.contains('on')&&q.getAttribute('aria-expanded')==='false';
      zet('pending',ok?'ok':'fout');zet('pending-text',melding&&melding.textContent.trim());
...
try{
  ...
  const r=spawnSync(browser,["--headless=new","--no-sandbox","--disable-gpu","--disable-dev-shm-usage","--allow-file-access-from-files","--window-size=390,844","--virtual-time-budget=1900","--dump-dom","file://"+bestand],{encoding:"utf8",maxBuffer:24*1024*1024});
  ...
  if(waarde("done")!=="ok"||waarde("pending")!=="ok"||waarde("result")!=="ok"||waarde("short-query-clean")!=="ok"){
    throw new Error(`Location-search browsercheck fout: pending=${waarde("pending")} pendingText=${waarde("pending-text")} result=${waarde("result")} resultMessage=${waarde("result-message")} short=${waarde("short-query-clean")} done=${waarde("done")}`);
  }
  console.log("Location-search browser 390px: Plaatsen zoeken zichtbaar tijdens geocoder-wacht, daarna schoon vervangen door resultaat en korte query ruimt alles op.");
