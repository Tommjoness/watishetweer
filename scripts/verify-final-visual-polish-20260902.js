...
function luminantie(hex){const h=hex.replace("#","");const rgb=[0,2,4].map(i=>parseInt(h.slice(i,i+2),16)/255).map(c=>c<=.04045?c/12.92:Math.pow((c+.055)/1.055,2.4));return .2126*rgb[0]+.7152*rgb[1]+.0722*rgb[2];}
function contrast(a,b){const x=luminantie(a),y=luminantie(b);return (Math.max(x,y)+.05)/(Math.min(x,y)+.05);}
...
for(const p of weer){
  ...
  const rood=/html\[data-thema="rood"\]\{--ink:(#[0-9A-Fa-f]{6});--ink-70:(#[0-9A-Fa-f]{6});--ink-45:(#[0-9A-Fa-f]{6});--ink-25:(#[0-9A-Fa-f]{6})/.exec(html);
  eis(rood,rel+": rode contrasttokens ontbreken");
  for(const kleur of rood.slice(1)){
    eis(contrast(kleur,"#0D0303")>=4.5,rel+`: rood teksttoken ${kleur} haalt geen 4.5:1 op sheet`);
    eis(contrast(kleur,"#080202")>=4.5,rel+`: rood teksttoken ${kleur} haalt geen 4.5:1 op paper`);
  }
...
console.log(`Final visual polish artifact groen op ${weer.length} weerpagina's: desktopbalans behouden; mobile urenpreview, safe areas, rood AA-contrast, headerhiërarchie en Nachtzicht-semantiek geborgd.`);
