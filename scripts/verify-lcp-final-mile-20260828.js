...
assert.ok(html.includes("if(mobieleLuchtRenderUitgesteld)return;"),"async luchtkwaliteitsrender kan de mobiele LCP-volgorde niet inhalen");
...
assert.ok(/const stap4=\(\)=>\{\s*mobieleLuchtRenderUitgesteld=false;\s*lucht\(\);nuTimerStart\(\);klokTimerStart\(\);\s*\};/.test(html),"luchtkwaliteit wordt pas na Nachtzicht vrijgegeven en timers starten daarna");
...
console.log("LCP final-mile 20260828: mobiele briefingpaint, deterministische deferred luchtkwaliteit, forecast-preconnect en direct desktoprenderpad geborgd.");
