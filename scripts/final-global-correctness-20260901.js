...
const providerId=r=>r&&r.id!==null&&r.id!==undefined&&String(r.id).trim()!==""?String(r.id).trim():null;
...
function dedupliceerZoekresultaten(resultaten,max=Infinity){
  const uit=[],gezienGeo=new Set(),gezienIds=new Set(),limiet=Number.isFinite(Number(max))?Math.max(0,Math.floor(Number(max))):Infinity;
  for(const r of Array.isArray(resultaten)?resultaten:[]){
    ...
    const geo=zoekSleutel(r),id=providerId(r);
    if(!geo||gezienGeo.has(geo)||(id&&gezienIds.has(id)))continue;
    gezienGeo.add(geo);if(id)gezienIds.add(id);uit.push(r);if(uit.length>=limiet)break;
  }
  return uit;
}
...
function parseLokaal(iso){
  ...
  p.datum=m[1]+"-"+m[2]+"-"+m[3];p.tijd=m[4]+":"+m[5];p.civiel=Date.UTC(p.jaar,p.maand-1,p.dag,p.uur,p.minuut);
  return p;
}
function tijdDelen(epoch,tijdzone){
  try{
    const f=new Intl.DateTimeFormat("en-CA",{timeZone:tijdzone,year:"numeric",month:"2-digit",day:"2-digit",hour:"2-digit",minute:"2-digit",hourCycle:"h23"}),o={};
    for(const x of f.formatToParts(new Date(epoch)))if(x.type!=="literal")o[x.type]=x.value;
...
function tijdzoneKandidaten(iso,tijdzone){
  ...
  for(let h=-36;h<=36;h+=6){const probe=p.civiel+h*3600000,parts=tijdDelen(probe,tijdzone);if(!parts)continue;offsets.add(Math.round((Date.UTC(parts.jaar,parts.maand-1,parts.dag,parts.uur,parts.minuut)-probe)/60000)*60000);}
  const uit=[];for(const off of offsets){const epoch=p.civiel-off;if(zelfdeLokaal(p,tijdDelen(epoch,tijdzone)))uit.push(epoch);}
  return [...new Set(uit)].sort((a,b)=>a-b);
}
function intervalEpoch(startIso,eindIso,tijdzone,referentieMs){
  const a=tijdzoneKandidaten(startIso,tijdzone),b=tijdzoneKandidaten(eindIso,tijdzone);if(!a.length||!b.length)return null;
  const kandidaten=[];for(const s of a)for(const e of b)if(e>s&&e-s<=30*3600000)kandidaten.push({start:s,eind:e});if(!kandidaten.length)return null;
  if(Number.isFinite(Number(referentieMs))){const r=Number(referentieMs);kandidaten.sort((x,y)=>Math.abs((x.start+x.eind)/2-r)-Math.abs((y.start+y.eind)/2-r));}else kandidaten.sort((x,y)=>x.start-y.start||x.eind-y.eind);
  return kandidaten[0];
}
...
function nachtVensterTijdsvorm(tekst,opt={}){
  ...
  const eindDatum=d.eind<=d.start?datumVerschuif(datum,1):datum;if(!eindDatum)return d.label+": "+d.start+"–"+d.eind+".";
  const startIso=datum+"T"+d.start,eindIso=eindDatum+"T"+d.eind,tz=String(opt.tijdzone||""),interval=intervalEpoch(startIso,eindIso,tz,opt.nuEpochMs),nuEpoch=huidigEpoch(opt.nuDatumTijd,tz,opt.nuEpochMs);
  let toestand="toekomstig";if(interval&&nuEpoch!==null)toestand=nuEpoch>=interval.eind?"verstreken":nuEpoch>=interval.start?"actief":"toekomstig";else{const s=parseLokaal(startIso),e=parseLokaal(eindIso);if(s&&e)toestand=nu.civiel>=e.civiel?"verstreken":nu.civiel>=s.civiel?"actief":"toekomstig";}
  if(toestand==="verstreken")return d.label+" was "+d.start+"–"+d.eind+".";if(toestand==="actief")return d.label+": nu tot "+d.eind+".";return d.label+": "+d.start+"–"+d.eind+".";
}
...
function corrigeerGradenTekst(tekst){return String(tekst==null?"":tekst).replace(/(^|[^\d,.-])(-?1) graden\b/g,"$1$2 graad");}

const WMO={0:"Onbewolkt",1:"Overwegend zonnig",2:"Half bewolkt",3:"Bewolkt",45:"Mist",48:"Rijpmist",51:"Lichte motregen",53:"Motregen",55:"Dichte motregen",56:"Lichte ijzelmotregen",57:"IJzelmotregen",61:"Lichte regen",63:"Regen",65:"Zware regen",66:"Lichte ijzel",67:"IJzel",71:"Lichte sneeuw",73:"Sneeuw",75:"Zware sneeuw",77:"Sneeuwkorrels",80:"Lichte regenbuien",81:"Regenbuien",82:"Zware regenbuien",85:"Lichte sneeuwbuien",86:"Zware sneeuwbuien",95:"Onweer",96:"Onweer met hagel",99:"Zwaar onweer met hagel"};
function neerslagCode(code){const c=num(code);return c!==null&&((c>=51&&c<=82)||(c>=85&&c<=86)||c===95||c===96||c===99);}
function dagBasis(a,basis){
  const code=num(a&&a.code),mm=num(a&&a.hoeveelheid);let t=code!==null&&WMO[code]?WMO[code]:String(basis||"Verwachting");
  if(neerslagCode(code)&&mm!==null&&mm>=5){t=t.replace(/^Lichte\s+/i,"");if(t)t=t.charAt(0).toUpperCase()+t.slice(1);}return t;
}
function dagKansTekst(a,basis){
  const b=dagBasis(a,basis),k=num(a&&a.kans),soort=b.toLowerCase(),tijd=String(a&&a.eersteTijd||"");if(k===null)return b;
  ...
  if(!neerslagCode(a&&a.code))return b;if(k===0)return "Overwegend droog";if(k<=9)return "Zeer kleine kans op "+soort+dagdeel;if(k<=29)return "Kleine kans op "+soort+dagdeel;if(k<=69)return b+" mogelijk"+dagdeel;if(k<=89)return "Grote kans op "+soort+dagdeel;return "Zeer grote kans op "+soort+dagdeel;
}
function dagHoeveelheidStatus(kans,mm){const k=num(kans),m=num(mm);if(m!==null&&m<0)return "niet beschikbaar";if(m===null)return k!==null&&k>0?"hoeveelheid onzeker":"niet beschikbaar";if(m===0)return k===0?"Droog":"0,0 mm";if(m<0.005)return "spoor";if(m<0.05)return "<0,05 mm";return m.toFixed(1).replace(".",",")+" mm";}
...
function nachtAdvies(score,reden){
  const s=num(score),r=String(reden||"").trim().replace(/[.!?]+$/g,"");if(s===null)return r?"Geen gunstig kijkvenster door "+r+".":"Onvoldoende gegevens voor een betrouwbare beoordeling.";
  if(s>=7){if(/maan/i.test(r))return "De totale zichtscore is hoog; maanlicht maakt de hemel minder donker.";if(r)return "De totale zichtscore is hoog, maar "+r+" onderbreekt een langer optimaal kijkvenster.";return "De totale zichtscore is hoog.";}
