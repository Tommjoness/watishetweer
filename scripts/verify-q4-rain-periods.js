...
ok(html.includes("function q4PeriodeBedragLabels(g,perioden,eersteY,font)"),"desktopperiodehoeveelheden houden hun botsingsbewuste layouthulp");
ok(html.includes('data-q4-rain-period-amount')&&html.includes('q4Mm(p.som)+" mm"'),"desktop toont per regenperiode de berekende totaalhoeveelheid");
ok(html.includes("function q4PeriodeRandTekst(g,p)"),"regenperiode heeft één zichtbare kloktijdhelper");
ok(html.includes('return {van:q4Tijd(van),tot:q4Tijd(tot)};'),"desktop-tijdlabels tonen uitsluitend kloktijden zonder weekdag");
ok(html.includes("function q4PeriodeRandLabels(g,perioden,y,font)"),"desktop-tijdlabels hebben een eigen botsingsbewuste layouthulp");
ok(html.includes('if(!g||g.n>25)return {labels:[],rijen:0'),"desktop-tijdlabels blijven beperkt tot de 24-uursweergave");
ok(html.includes('const splitMin=Math.max(52,startBreedte+eindBreedte+10);')&&html.includes('if(span>=splitMin)'),"desktop split-versus-compact volgt de werkelijke bracketbreedte");
...
ok(html.includes('const compactTekst=tekst.van+"–"+tekst.tot')&&html.includes('labels.push({index,soort:"range"'),"korte desktopperiode valt terug op één compacte klokrange");
ok(html.includes('const links=2,rechts=g.W-2'),"tijdlabels gebruiken de echte SVG-rand als clipgrens");
ok(html.includes('label.setAttribute("text-anchor",item.anchor||"middle")'),"desktop-tijdlabels gebruiken passende SVG-ankers");
ok(html.includes('data-q4-rain-period-"+item.soort'),"desktop split- en compacte tijdlabels krijgen regressie-attributen");
ok(html.includes("function q4MobielePeriodeLabels(g,perioden,y,font)"),"mobiel heeft één eigen compacte periode-labelowner");
ok(html.includes('tijdKort(van)+"–"+tijdKort(tot)+" · "+q4Mm(p.som)+" mm"'),"mobiel combineert tijdvak en hoeveelheid op één regel");
ok(html.includes('data-q4-rain-period-mobile'),"mobiele gecombineerde regenlabels zijn expliciet herkenbaar");
ok(html.includes('if(g.M){\n    mobiel.labels.forEach'),"mobiel rendert niet tevens de losse desktop tijd- en mm-rijen");
...
ok(html.includes('horizontaal.setAttribute("aria-label",q4PeriodeTijdvak(g,p)+" · "+q4Mm(p.som)+" mm")'),"bracket houdt het volledige toegankelijke tijdvak plus hoeveelheid");
...
ok(html.includes('const nieuwH=Math.max(basisH,laatsteLabelY+25);'),"grafiekhoogte volgt alleen de werkelijk gerenderde mobiele of desktoplabelrijen");
ok(html.includes('compact tijdvak met de verwachte hoeveelheid'),"toegankelijke grafiekuitleg benoemt de compacte mobiele presentatie");
