...
ok(html.includes("function q4PeriodeRandTekst(g,p)"),"bracketuiteinden hebben één zichtbare tijdhelper");
ok(html.includes('return {van:q4Tijd(van),tot:q4Tijd(tot)};'),"zichtbare bracketuiteinden tonen uitsluitend kloktijden zonder weekdag");
...
ok(html.includes('const nieuwH=Math.max(basisH,laatsteBedragY+17+8);'),"grafiekhoogte reserveert alleen ruimte voor bracketlabels en mm-bedragen");
ok(html.includes('Bij iedere regenperiode staan begin- en eindtijd en verwachte hoeveelheid.'),"toegankelijke grafiekuitleg beschrijft de compacte bracketlabels");
