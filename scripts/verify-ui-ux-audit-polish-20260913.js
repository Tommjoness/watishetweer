...
for(const p of htmlBestanden(OUT)){
  ...
  assert(html.includes('html body footer:nth-of-type(n) > span.bron.bron-bronnen{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;grid-auto-flow:row!important;align-items:stretch!important;width:100%;column-gap:16px!important;row-gap:1px!important')&&html.includes('footer .bron-bronnen .bronlabel{grid-column:1 / -1;margin:0!important;text-align:center}'),rel+": mobiele bronlijst heeft geen harde 2-koloms row-flow override op de late runtime-flexowner");
  assert(html.includes('footer .bron-bronnen .bronitem{min-width:0}'),rel+": bronitems missen hun neutrale gridbasis");
  assert(html.includes('footer .bron-bronnen .bronitem.wiw-source-last-odd{grid-column:1 / -1!important;justify-self:stretch}')&&html.includes('footer .bron-bronnen .bronitem.wiw-source-last-odd a{width:calc(50% - 8px)!important;margin-inline:auto!important}'),rel+": oneven laatste bron spant niet gecentreerd over beide kolommen");
  assert(html.includes('footer .bron-bronnen .bronitem a{display:flex!important;align-items:center!important;justify-content:center!important;width:100%;min-height:44px!important;margin:0!important;padding:0!important;line-height:1.2!important;text-align:center;border:0!important;border-bottom:1px solid var(--rule-soft)!important;box-shadow:none!important;background:transparent!important}'),rel+": mobiele bronlinks verliezen hun 44px touchdoel of uniforme zachte scheidingslijn");
  ...
  assert(html.includes('footer > span.bron:nth-of-type(2){grid-column:1 / -1!important;grid-row:2!important;justify-self:center!important;width:calc(100% + 42px);max-width:calc(100vw - 8px);min-height:0!important;margin:4px 0 0!important;padding:2px 0!important;line-height:1.35!important}'),rel+": disclaimer mist het rustigere mobiele landingsritme");
  ...
  assert(html.includes('.footer-contact{grid-column:1 / -1;justify-self:center;display:flex;flex-wrap:wrap;align-items:center;justify-content:center;width:calc(100% + 42px);max-width:calc(100vw - 8px);column-gap:8px;row-gap:0;margin-top:8px!important;line-height:1.3}'),rel+": supportregel mist de bedoelde mobiele ademruimte");
  assert(html.includes('.chip.on{border-color:var(--rule)!important;background:var(--sheet)!important;color:var(--ink)!important;box-shadow:inset 0 -2px 0 var(--ink-45)!important}'),rel+": actieve bewaarde plaats blijft mobiel te warning-achtig");
  assert(html.includes('.stats .eyebrow{font-size:9.5px!important;letter-spacing:.075em!important;line-height:1.15!important;min-height:2.3em!important}'),rel+": lange mobiele metrieklabels missen het rustige kopritme na final-top-grid-herplaatsing");
  ...
  assert(html.includes('html body > .seo-plaatsnav .seo-plaatsnav-inner{grid-template-columns:275px minmax(0,1fr)!important}')&&html.includes('html body > .seo-plaatsnav .seo-plaatsnav-links{gap:4px 8px!important}'),rel+": desktopplaatsenkop mist ruimte vóór de eerste link");
  ...
  assert(html.includes('.wiw-hour-table thead th{white-space:normal!important;overflow-wrap:anywhere!important;line-height:1.2!important}'),rel+": mobiele uurtabelkop kan bij tekstzoom afkappen");
