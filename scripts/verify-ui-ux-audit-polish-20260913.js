...
for(const p of htmlBestanden(OUT)){
  ...
  assert(html.includes('html body footer:nth-of-type(n){margin-top:8px!important;padding-top:2px!important;padding-bottom:0!important;display:grid!important;grid-template-columns:minmax(0,1fr) max-content max-content max-content minmax(0,1fr)!important'),rel+": mobiele footer heeft geen expliciete compacte grid-owner");
  assert(html.includes('footer .bron-bronnen{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important')&&html.includes('footer .bron-bronnen .bronlabel{grid-column:1 / -1;margin:0 0 1px!important}'),rel+": mobiele bronlijst is niet gelijkmatig in twee kolommen verdeeld");
  assert(html.includes('footer .bron-bronnen .bronitem a{display:flex!important;align-items:center!important;justify-content:flex-start!important;min-height:44px!important;margin:0!important;padding:0!important}'),rel+": mobiele bronlinks verliezen hun 44px touchdoel of erven oude horizontale spacing");
  ...
  assert(html.includes('footer > span.bron:nth-of-type(2){grid-column:1 / -1!important;grid-row:2!important;justify-self:center!important;width:calc(100% + 24px);max-width:calc(100vw - 40px);min-height:0!important;margin:2px 0 0!important;line-height:1.45!important}'),rel+": disclaimer benut mobiel niet de bredere compacte leesregel");
  assert(html.includes('@media(max-width:370px){\n  html body footer:nth-of-type(n){grid-template-columns:minmax(0,1fr) minmax(0,1fr)!important}'),rel+": smalle mobielfooter mist de veilige tweekolomsfallback");
  assert(html.includes('.footer-contact{grid-column:1 / -1;justify-self:center;display:flex;flex-wrap:wrap;align-items:center;justify-content:center;column-gap:6px;row-gap:0;margin-top:0!important;line-height:1.3}'),rel+": mobiel supportcontact houdt nog extra bovenmarge of valt buiten de footergrid");
  ...
  assert(html.includes('body > .seo-plaatsnav{margin-top:10px!important;padding-top:12px!important;padding-bottom:0!important}'),rel+": overgang naar populaire plaatsen is niet compact of gelijkmatig");
