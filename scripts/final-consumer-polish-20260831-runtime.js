...
function verfijnGrafiekTypografie(){const svg=document.getElementById("chart");if(!svg)return;const mobiel=window.innerWidth<760;svg.querySelectorAll("text[font-size]").forEach(el=>{const fs=Number(el.getAttribute("font-size"));if(!Number.isFinite(fs)||fs<=0)return;const tekst=String(el.textContent||"").trim();const temp=/^-?\d+°$/.test(tekst)||/^nu\s+-?\d+°$/i.test(tekst);const telefoon=window.innerWidth<=430,factor=temp?(telefoon?0.72:mobiel?0.94:window.innerWidth<1100?0.78:0.80):(telefoon?0.84:mobiel?0.92:0.88);el.setAttribute("font-size",String(Math.max(7.5,Math.round(fs*factor*10)/10)));if(temp){el.setAttribute("opacity",mobiel?"0.76":"0.82");if(el.hasAttribute("stroke-width"))el.setAttribute("stroke-width",mobiel?"2":"2.4");}});svg.querySelectorAll("circle[data-temp-index]").forEach(el=>el.setAttribute("opacity",mobiel?"0.5":"0.62"));svg.dataset.desktopTypography=mobiel?"calm-mobile":"compact";}
