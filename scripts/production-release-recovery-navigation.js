       ...
       huidige app + bootstrap, en nooit onze stale shell. */
    {
      ...
      try{
        ...
        const injectie=await workerNetwerk.child.send("Runtime.evaluate",{
          expression:'(()=>{const trace=[];globalThis.__WIW_E2E_TRACE__=trace;if(typeof globalThis.__WIW_E2E_ORIGINAL_FETCH__!=="function")globalThis.__WIW_E2E_ORIGINAL_FETCH__=globalThis.fetch.bind(globalThis);globalThis.fetch=input=>{trace.push({soort:"network",url:String(input&&input.url||input),mode:input&&input.mode||null,cache:input&&input.cache||null});return Promise.reject(new TypeError("WIW E2E network unavailable"));};if(!globalThis.__WIW_E2E_ORIGINAL_CACHE_MATCH__){globalThis.__WIW_E2E_ORIGINAL_CACHE_MATCH__=CacheStorage.prototype.match;CacheStorage.prototype.match=async function(request,options){try{const response=await globalThis.__WIW_E2E_ORIGINAL_CACHE_MATCH__.call(this,request,options);let retour=response,genormaliseerd=false;if(response&&/__sw_install=/.test(response.url||"")&&/text\\/html/i.test(response.headers.get("content-type")||"")){const headers=new Headers(response.headers);headers.delete("content-encoding");headers.delete("content-length");retour=new Response(await response.arrayBuffer(),{status:response.status,statusText:response.statusText,headers});genormaliseerd=true;}trace.push({soort:"cache",url:String(request&&request.url||request),cacheName:options&&options.cacheName||null,hit:!!response,status:response&&response.status||null,responseUrl:response&&response.url||null,genormaliseerd,retourUrl:retour&&retour.url||null});return retour;}catch(error){trace.push({soort:"cache-error",fout:String(error)});throw error;}};}if(!globalThis.__WIW_E2E_ORIGINAL_RESPOND_WITH__){globalThis.__WIW_E2E_ORIGINAL_RESPOND_WITH__=FetchEvent.prototype.respondWith;FetchEvent.prototype.respondWith=function(value){const request=this.request;trace.push({soort:"respondWith",url:request&&request.url||null,mode:request&&request.mode||null});return globalThis.__WIW_E2E_ORIGINAL_RESPOND_WITH__.call(this,Promise.resolve(value).then(response=>{trace.push({soort:"response",status:response&&response.status||null,url:response&&response.url||null,type:response&&response.type||null});return response;},error=>{trace.push({soort:"response-error",fout:String(error)});throw error;}));};}return true;})()',
          returnByValue:true
        });
