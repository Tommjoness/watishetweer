...
for(const p of htmlBestanden(OUT)){
  ...
  eis(/\.wiw-hour-table th:nth-child\(3\),\.wiw-hour-table td:nth-child\(3\)\{width:29%!important;text-align:center!important\}/.test(html),`${rel}: temperatuurkolom is niet met behoud van breedte gecentreerd`);
  eis(/\.wiw-hour-table th:nth-child\(4\),\.wiw-hour-table td:nth-child\(4\)\{width:28%!important\}/.test(html),`${rel}: neerslagkolom heeft niet de gebalanceerde breedte`);
  ...
  eis(html.includes('.final-top-grid #t{font-size:clamp(90px,6.5vw,102px)!important;line-height:.86!important}'),`${rel}: actuele temperatuur gebruikt niet de actuele final-top-grid-selector of de rustigere maat`);
  eis(html.includes('.final-top-grid .deg{font-size:26px!important;margin-top:4px!important}'),`${rel}: temperatuurunit sluit niet aan op de rustigere hero`);
...
console.log(`Uurpaneelcleanup geverifieerd op ${geraakt} weerartifacts: volledige Temperatuur-kop, geometrisch gecentreerde temperatuurkolom, rustigere actuele temperatuur op de actuele hero-selector, geen los Vandaag/Morgen-label en overige rijke data intact.`);
