...
for(const p of htmlBestanden(OUT)){
  ...
  assert(!/<button[^>]*id="thema"[^>]*class="wiw-theme-switch"/.test(html),rel+": verouderde binaire switch overschrijft de weather UI-shell");
  assert(html.includes('id="thema-auto"')&&html.includes('data-thema-keuze="auto"'),rel+": Auto-resetkeuze ontbreekt na late SEO/themapersistentielaag");
  ...
  assert(html.includes('ls.set("weerbriefing.thema","auto")')&&html.includes('ls.set("weerbriefing.thema",handmatig||'),rel+": expliciete Licht/Donker/Auto-keuze kan niet meer worden opgeslagen");
