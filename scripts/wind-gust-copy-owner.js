...
function weatherNowWindstootTekst(pg,nu,dag,vak){
  ...
  const tussen=tijdvak?tijdvak.replace("–"," en "):"het piekuur";
  const dagInZin=dagNaam?dagNaam.charAt(0).toLowerCase()+dagNaam.slice(1):"eerder";
  if(String(pg.t)>String(nu||"")){
    if(/^Vandaag$/i.test(dagNaam))return `De hoogste windstoot wordt vandaag tussen ${tussen} verwacht: ${waarde} km/u.`;
    if(/^Morgen$/i.test(dagNaam))return `De hoogste windstoot wordt morgen tussen ${tussen} verwacht: ${waarde} km/u.`;
    return `De hoogste windstoot wordt ${dagInZin} tussen ${tussen} verwacht: ${waarde} km/u.`;
  }
  if(/^Vandaag$/i.test(dagNaam))return `De hoogste windstoot werd vandaag tussen ${tussen} verwacht: ${waarde} km/u.`;
  if(/^Gisteren$/i.test(dagNaam))return `De hoogste windstoot werd gisteren tussen ${tussen} verwacht: ${waarde} km/u.`;
  return `De hoogste windstoot werd ${dagInZin} tussen ${tussen} verwacht: ${waarde} km/u.`;
}
