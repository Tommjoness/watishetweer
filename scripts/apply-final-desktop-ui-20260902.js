...
const FOOTER_GAP_OLD="gap:4px 16px!important";
const FOOTER_GAP_NEW="gap:0 16px!important";
const FOOTER_PADDING_OLD="padding-top:10px!important";
const FOOTER_PADDING_NEW="padding-top:4px!important";
...
function compacteerFooterRuntime(bron){let uit=String(bron);const gapAantal=uit.split(FOOTER_GAP_OLD).length-1;if(gapAantal!==1)throw new Error("Finale desktop-UI footergapanker verwacht exact één keer; gevonden "+gapAantal);uit=uit.replace(FOOTER_GAP_OLD,FOOTER_GAP_NEW);const paddingAantal=uit.split(FOOTER_PADDING_OLD).length-1;if(paddingAantal!==1)throw new Error("Finale desktop-UI footerpaddinganker verwacht exact één keer; gevonden "+paddingAantal);return uit.replace(FOOTER_PADDING_OLD,FOOTER_PADDING_NEW);}
const RUNTIME=compacteerFooterRuntime(herstelDrukOwnerRuntime(fs.readFileSync(path.join(__dirname,"final-desktop-ui-runtime-20260902.js"),"utf8")));
...
module.exports={OUT,START,MARKER,RUNTIME,PRESSURE_OLD,PRESSURE_NEW,FOOTER_GAP_OLD,FOOTER_GAP_NEW,FOOTER_PADDING_OLD,FOOTER_PADDING_NEW,herstelDrukOwnerRuntime,compacteerFooterRuntime,weerBestand,pasToe,main};