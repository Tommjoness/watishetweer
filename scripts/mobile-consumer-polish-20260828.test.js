...
const plan=ux.mobieleTemperatuurLabelPlan(uren25,temp,24);
assert.deepEqual(plan.ankers,ankers,"temperatuurplan gebruikt exact dezelfde verplichte ankers als de uuras");
assert(plan.extrema.every(e=>!plan.ankers.includes(e.i)),"extremum dat samenvalt met een verplicht anker wordt niet dubbel gepland");
assert.deepEqual(ux.lokaleTemperatuurExtrema([10,12,12,11,9,9,10],24).map(e=>[e.i,e.type]),[[1,"piek"],[4,"dal"]],"plateaus krijgen exact één deterministisch piek/dal-label");
assert.deepEqual(ux.lokaleTemperatuurExtrema([10,11,11,12],24),[],"vlak monotone plateauzone is geen vals extremum");
...
assert(js.includes('data-mobile-temp-missing-anchors')&&js.includes('data-mobile-temp-dropped-extrema'),"verplichte ankers en alleen optioneel vervallen extrema zijn afzonderlijk traceerbaar");
