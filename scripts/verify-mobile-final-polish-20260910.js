...
for(const p of htmlBestanden(OUT)){
  ...
  eis(html.includes('footer a[href="/over/"]{white-space:nowrap}'),`${rel}: compacte Over-link kan mobiel nog onrustig afbreken`);
  eis(!html.includes('footer a[href="/over/"] b{display:inline-block;margin-right:3px}'),`${rel}: verouderde merkseparatorstijl staat nog in het artifact`);
...
console.log(`Mobiele final-polish geverifieerd in ${gecontroleerd} artifacts: DST-veilige providerklokgrens, geen verstreken uurstempels, zachte fixed-headerovergang, compactere uurtitel en compacte Over-link.`);
