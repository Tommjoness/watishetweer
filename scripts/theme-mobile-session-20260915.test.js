...
assert(!CSS.includes("display:contents!important"),"de semantische switch mag geen renderbox van 0px krijgen");
assert.equal((CSS.match(/#thema-switch\{[\s\S]*?grid-column:1 \/ -1!important;[\s\S]*?grid-template-columns:repeat\(3,minmax\(0,1fr\)\)!important;[\s\S]*?overflow:visible!important/g)||[]).length,2,"de switch moet op desktop en mobiel de volledige driewegbediening dragen zonder het rechtersegment af te knippen");
assert.equal((CSS.match(/\.wiw-theme-moon\{[\s\S]*?grid-column:3!important;[\s\S]*?position:static!important/g)||[]).length,2,"Donker moet als echt derde gridsegment binnen de switch staan");
...
assert.equal((CSS.match(/#thema-switch\{[\s\S]*?z-index:1!important/g)||[]).length,2,"de volledige switchlaag moet onder de aparte Auto-knop blijven");
assert(CSS.includes(".wiw-theme-auto{\n    grid-column:2!important"),"Auto staat niet in het middelste segment");
assert(CSS.includes(".wiw-theme-moon{\n    grid-column:3!important;\n    grid-row:1!important;\n    position:static!important"),"Donker staat niet als zichtbaar derde gridsegment");
