...
const vanaf="2026-08-15T08:30:00Z";
const tot="2026-08-15T11:05:00Z";
const queries=[
...
for(const q of queries){
  ...
  u.searchParams.set("time",vanaf+"/"+tot);
  const r=await fetch(u,{headers:{"user-agent":"watishetweer.nl point validation"},signal:AbortSignal.timeout(10000)});
  const text=await r.text();
  let parsed=null;
  try{parsed=JSON.parse(text);}catch{}
  console.log("KNMI_POINT",JSON.stringify({dataset:q.dataset,layer:q.layer,status:r.status,contentType:r.headers.get("content-type"),parsed,raw:text.slice(0,5000)}));
