...
for(const p of weer){
  ...
  eis(html.includes('<th scope="col">Kans</th><th scope="col">Neerslag</th>'),rel+": per-uur neerslagkolommen ontbreken");
  eis(!html.includes('<th scope="col">Gevoelstemperatuur</th>'),rel+": oude gevoelstemperatuurkolom staat nog in artifact");
...
console.log(`Final release hardening geverifieerd op ${weer.length} weerartifacts: cache-identiteit, uitgeschakelde partial preview, coherente laad/fout/retry-state, per-uur neerslagkolommen, finale-DOM desktopgrids en repository-URL zijn geborgd.`);
