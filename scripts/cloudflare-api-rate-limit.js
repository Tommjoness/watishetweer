...
function foutUitBody(status, body) {
  ...
  const permissieHint = Number(status) === 403
    ? " De Cloudflare-token heeft voor deze zone ook WAF/Rulesets read+write nodig."
    : "";
...
function vindEigenRegel(rules) {
  ...
  return rules.find(regel => regel && (regel.ref === RULE_REF || regel.description === RULE_DESCRIPTION)) || null;
}
...
async function controleerApiRateLimit({ accountId, token, project = PROJECT, fetchImpl = fetch }) {
  ...
  if (!ruleset) return { actief: false, reden: "http_ratelimit-ruleset ontbreekt", zoneTag, rulesetId: null, ruleId: null };
  const regel = vindEigenRegel(ruleset.rules);
  return {
...
async function borgApiRateLimit({ accountId, token, project = PROJECT, fetchImpl = fetch }) {
  ...
  if (!ruleset) {
    ...
    const bestaand = vindEigenRegel(rules);
    ...
    const basis = `https://api.cloudflare.com/client/v4/zones/${zoneTag}/rulesets/${ruleset.id}`;
    if (bestaand && bestaand.id) {
      ...
      if (rules.length > 0) {
        throw new Error("Er bestaat al een andere Cloudflare rate-limitregel. Die wordt bewust niet automatisch overschreven.");
      }
  ...
  const bevestigd = ruleset && vindEigenRegel(ruleset.rules);
