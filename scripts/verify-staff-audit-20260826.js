...
for(const [naam,html] of [["root",rootHtml],["Amsterdam-route",routeHtml]]){
  ...
  for(const vereist of [
    ...
    'id="chartdata"','<table>','document.querySelectorAll("#days .dag-neerslagnotitie")',
  ...
  if(!metaCsp)throw new Error(naam+": meta-CSP ontbreekt");
  if(!metaCsp.includes("script-src 'self' 'unsafe-inline' "+insights))throw new Error(naam+": Cloudflare Insights ontbreekt uit productie meta-CSP");
  if(!metaCsp.includes("connect-src 'self' https://api.open-meteo.com https://air-quality-api.open-meteo.com https://geocoding-api.open-meteo.com https://api.bigdatacloud.net"))throw new Error(naam+": connect-src is inhoudelijk gewijzigd");
  if(/script-src[^;]*\*/.test(metaCsp))throw new Error(naam+": script-src bevat een wildcard");
  if(/connect-src[^;]*cloudflareinsights\.com/.test(metaCsp))throw new Error(naam+": automatische RUM heeft onnodig een externe connect-src gekregen");
}
...
console.log(`Staff-audit build groen: root + Amsterdam-route, history, invalid URL, 40px touch target, grafiektabel, warningmapping, daggebonden neerslagduiding, share-image en Cloudflare Insights CSP. Root HTML ${rauw} bytes, gzip ${gzip} bytes.`);
