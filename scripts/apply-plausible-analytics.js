...
const CONNECT_SOURCE="https://plausible.io";
const SCRIPT_SRC="/plausible-analytics.js";
const SCRIPT_TAG=`<script src="${SCRIPT_SRC}" defer data-analytics="plausible"></script>`;
...
function verruimConnectSrc(policy){
  ...
  if(defaultIndex>=0){
    ...
    if(!bronnen.length)throw new Error("HTML-CSP heeft een ongeldige lege default-src; Plausible capture kan niet veilig worden toegestaan.");
...
function pasHtmlAan(html){
  ...
  const bestaande=(uit.match(/<script\b[^>]*\bsrc=["']\/plausible-analytics\.js["'][^>]*><\/script>/gi)||[]).length;
  if(bestaande>1)throw new Error("Plausible analytics-script staat dubbel in HTML-artifact.");
...
function pasArtifactAan(root=path.join(__dirname,"..","public")){
  const asset=path.join(root,"plausible-analytics.js");
  if(!fs.existsSync(asset))throw new Error("plausible-analytics.js ontbreekt in publieke buildoutput.");
  ...
  if(!indexHtml.includes(SCRIPT_TAG))throw new Error("Plausible analytics-script ontbreekt na artifactbewerking in index.html.");

  const deliveryActief=indexHtml.includes(DELIVERY_META);
  const metaStaatToe=indexHtml.includes(CONNECT_SOURCE);
  if(!metaStaatToe){
    if(!deliveryActief)throw new Error("Plausible capture-origin ontbreekt in de meta-CSP van index.html.");
    if(!productieHeaderStaatToe())throw new Error("Plausible capture-origin ontbreekt in de productie-CSP-header na deliverymigratie.");
  }

  console.log(`plausible-analytics: ${bestanden.length} HTML-bestanden gecontroleerd, ${scripts} scripts actief, ${metas} meta-CSP's gezien, ${gewijzigd} bestanden aangepast.`);
