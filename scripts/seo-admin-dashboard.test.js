...
assert(html.includes('rel="apple-touch-icon" sizes="180x180" href="/admin/seo/apple-touch-icon.png"'),"SEO cockpit mist eigen PNG-homescreen-icon.");
assert(html.includes('rel="icon" type="image/svg+xml" href="/admin/seo/icon.svg"'),"SEO cockpit mist schaalbaar browsericoon.");
assert(fs.existsSync(path.join(root,"admin/seo/icon.svg")),"SEO cockpit SVG-icon ontbreekt.");
const seoPng=fs.readFileSync(path.join(root,"admin/seo/apple-touch-icon.png"));
assert.equal(seoPng.readUInt32BE(16),180,"SEO homescreen-PNG moet 180px breed zijn.");
assert.equal(seoPng.readUInt32BE(20),180,"SEO homescreen-PNG moet 180px hoog zijn.");
...
assert(api.includes('timeZone:"America/Los_Angeles"')&&api.includes("function searchConsoleDate"),"24-uursquery moet Search Console-datumgrenzen in Pacific Time berekenen.");
