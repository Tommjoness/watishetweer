...
for(const p of htmlBestanden(OUT)){
  ...
  eis(html.includes('const basisMinimum=window.innerWidth>=1440?29.5:29;')&&html.includes('const gemetenRijvloer=basisRijen.length?Math.max(basisMinimum,Math.max(...basisRijen.map(r=>r.getBoundingClientRect().height))):basisMinimum;')&&html.includes('const gewensteMinimumUren=window.innerWidth>=1440?10:8;')&&html.includes('const minimumUren=Math.min(gewensteMinimumUren,Math.floor((beschikbareRijhoogte+0.5)/gemetenRijvloer),tbody&&tbody.children.length||0);'),`${rel}: desktopcapaciteit gebruikt niet de echte gerenderde rijvloer met 1440px veiligheidsmarge`);
  eis(html.includes('tbody.children.length>minimumUren'),`${rel}: hoogtefilter kan de beschikbare achturenhorizon nog inkorten`);
  eis(html.includes('const maximaleKrimp=Math.max(0,(kleinsteRij-basisMinimum)/2);'),`${rel}: begrensde paddingcorrectie bewaakt de breedte-afhankelijke leesbaarheidsvloer niet`);
  eis(html.includes('document.documentElement.getBoundingClientRect();')&&html.includes('rest>0.25&&rijPadAanpassing<maxExtra')&&html.includes('const maxExtra=Math.max(4.5,laatsteRect.height/(zichtbareRijen.length*2)+0.5);'),`${rel}: subpixel-resthoogte wordt niet begrensd nagemeten en geabsorbeerd`);
  ...
  eis(/@media\(min-width:1280px\)\{[\s\S]*?\.wiw-chart-main\{min-height:340px!important\}[\s\S]*?\.wiw-hour-table td\{padding:calc\(1\.25px \+ var\(--wiw-hour-row-pad-extra,0px\)\) 4px!important\}/.test(html),`${rel}: gekoppelde 1280px-desktop mist de minimale hoogte of subpixelveiligheidsmarge voor acht volledige leesbare uurregels`);
  eis(/@media\(min-width:1440px\)\{[\s\S]*?\.wiw-chart-main\{min-height:384px!important\}/.test(html),`${rel}: ruime desktop mist de gemeten minimale grafiekhoogte voor tien leesbare uurregels`);
