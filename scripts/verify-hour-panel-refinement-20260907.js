...
for(const p of htmlBestanden(OUT)){
  ...
  eis(/#wiw-hour-panel h3\{[\s\S]*?margin-top:0!important;[\s\S]*?margin-bottom:4px!important/.test(html),`${rel}: compacte uurpaneelkop ontbreekt`);
  eis(/\.wiw-hour-table td\{[\s\S]*?padding-top:2px!important;[\s\S]*?padding-bottom:2px!important;[\s\S]*?line-height:16px!important/.test(html),`${rel}: compacte desktop-uurrij ontbreekt`);
  eis(/\.wiw-hour-table th\{[\s\S]*?padding-top:2px!important;[\s\S]*?padding-bottom:2px!important;[\s\S]*?line-height:13px!important/.test(html),`${rel}: compacte desktop-uurkop ontbreekt`);
...
console.log(`Uurpaneelrefinement geverifieerd op ${geraakt} weerartifacts: maximaal 17 desktopuren, 0 mm blijft 0 mm, desktop-CSS staat actief in head, masthead/SEO-inhoud hebben veilige insets en Nachtzicht gebruikt brede ruimte.`);
