...
for(const p of htmlBestanden(OUT)){
  ...
  eis(/\.wiw-hour-table td\{[\s\S]*?padding-top:calc\(2px \+ var\(--wiw-hour-row-pad-extra,0px\)\)!important;[\s\S]*?padding-bottom:calc\(2px \+ var\(--wiw-hour-row-pad-extra,0px\)\)!important;[\s\S]*?line-height:14px!important/.test(html),`${rel}: dynamisch verdeelde leesbare desktop-uurrij ontbreekt`);
...
console.log(`Uurpaneelrefinement geverifieerd op ${geraakt} weerartifacts: 24 uur bronkandidaten zonder vast zichtbaar rij-aantal, paneelhoogte volgt de grafiek, resterende sub-rijhoogte wordt gelijkmatig verdeeld over leesbare rijen, actuele Nu-context blijft behouden, 0 mm blijft 0 mm, desktop-CSS staat actief in head, masthead/SEO-inhoud hebben veilige insets en Nachtzicht gebruikt brede ruimte.`);