...
for(const p of htmlBestanden(OUT)){
  ...
  eis(/if\(S\.dag==null&&S\.bereik!==24\)\{aside\.style\.height="";return;\}[\s\S]*?aside\.style\.height="";[\s\S]*?const h=main\.getBoundingClientRect\(\)\.height;/.test(html),`${rel}: eerdere paneelhoogte wordt niet vóór grafiekmeting gewist`);
  eis(/\.wiw-chart-layout\{align-items:start!important\}/.test(html),`${rel}: grid kan uurkolom nog de grafiekhoogte laten bepalen`);
  eis(/\.wiw-chart-main,\.wiw-hour-panel\{align-self:start!important\}/.test(html),`${rel}: grafiek en uurpaneel zijn niet intrinsiek uitgelijnd`);
  ...
  eis(/\.wiw-hour-table td\{[\s\S]*?padding-top:calc\(5px \+ var\(--wiw-hour-row-pad-extra,0px\)\)!important;[\s\S]*?padding-bottom:calc\(5px \+ var\(--wiw-hour-row-pad-extra,0px\)\)!important;[\s\S]*?line-height:14px!important/.test(html),`${rel}: comfortabele hoogtegestuurde desktop-uurrij ontbreekt`);
  eis(/\.wiw-hour-table th\{[\s\S]*?padding-top:3px!important;[\s\S]*?padding-bottom:3px!important;[\s\S]*?line-height:12px!important/.test(html),`${rel}: leesbare desktop-uurkop ontbreekt`);
  ...
  eis(/#nights \.row\.night:not\(\.kop\) \.nmeta\.wide\{[\s\S]*?grid-template-columns:minmax\(0,1fr\) minmax\(180px,230px\)!important/.test(html),`${rel}: brede Nachtzicht-meta-indeling ontbreekt`);
...
console.log(`Uurpaneelrefinement geverifieerd op ${geraakt} weerartifacts: 24 uur bronkandidaten zonder vast zichtbaar rij-aantal, oude paneelhoogte wordt vóór hermeting gewist, griditems blijven intrinsiek, uurregels hebben een comfortabele minimumhoogte, resterende sub-rijhoogte wordt gelijkmatig verdeeld, actuele Nu-context blijft behouden, 0 mm blijft 0 mm en desktop-CSS staat actief in head.`);