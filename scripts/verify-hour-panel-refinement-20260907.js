...
for(const p of htmlBestanden(OUT)){
  ...
  eis(html.includes(UREN_NIEUW)&&!html.includes(UREN_OUD),`${rel}: desktoplimiet is niet exact 14 uur`);
  ...
  eis(/\.wiw-hour-table th,\.wiw-hour-table td\{[\s\S]*?padding-top:4px!important;[\s\S]*?padding-bottom:4px!important/.test(html),`${rel}: compacte desktop-uurrij ontbreekt`);
...
console.log(`Uurpaneelrefinement geverifieerd op ${geraakt} weerartifacts: maximaal 14 desktopuren, 0 mm blijft 0 mm, desktop-CSS staat actief in head, masthead/SEO-inhoud hebben veilige insets en Nachtzicht gebruikt brede ruimte.`);
