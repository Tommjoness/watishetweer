...
window.addEventListener('DOMContentLoaded',()=>{const zet=(k,v)=>document.body.setAttribute('data-audit-'+k,String(v));try{
  ...
  const zichtbarePlaatslinks=[...plaatsgrid.querySelectorAll('a')].filter(x=>cs(x).display!=='none'),utilityTargets=[over.querySelector('a'),privacy.querySelector('a'),details.querySelector('summary')].filter(Boolean),sourceTargets=[...bronnen.querySelectorAll('.bronitem a')],contactMail=contact.querySelector('a');
  ...
  const utilityRects=utilityTargets.map(x=>x.getBoundingClientRect()),sourceRects=sourceTargets.map(x=>x.getBoundingClientRect()),contactRect=contact.getBoundingClientRect();
  ...
  zet('source-columns',cs(bronnen).display==='grid'?(cs(bronnen).gridTemplateColumns||'').split(/\\s+/).filter(Boolean).length:0);
  ...
  try{
    ...
    if(breedte<=900){
      ...
      if(Number(v("source-columns"))!==2)throw new Error("bronlinks staan niet in twee gelijkmatige kolommen op "+breedte+"px: "+v("source-columns"));
      ...
      if(cssWidth>=390&&cssWidth<=430)console.log("footer-meting "+modus+" "+breedte+"px / CSS "+cssWidth+"px: hoogte="+v("footer-height")+"px, bronkolommen="+v("source-columns")+", bronhit="+v("source-hit-height")+"px, disclaimer="+v("disclaimer-width")+"x line-height "+v("disclaimer-line-height")+"px, source→disclaimer="+v("source-disclaimer-gap")+"px, utility→contact="+v("utility-contact-gap")+"px, utility-delta="+v("utility-row-delta")+"px");
      if(Number(v("footer-margin-top"))>8.5)throw new Error("mobiele footer houdt te veel bovenmarge op "+breedte+"px: "+v("footer-margin-top")+"px");
      if(Number(v("footer-padding-top"))>2.5)throw new Error("mobiele footer houdt te veel bovenpadding op "+breedte+"px: "+v("footer-padding-top")+"px");
      ...
      if(Number(v("place-margin-top"))>10.5||Number(v("place-padding-top"))>12.5)throw new Error("overgang naar populaire plaatsen blijft te ruim op "+breedte+"px: margin "+v("place-margin-top")+"px, padding "+v("place-padding-top")+"px");
